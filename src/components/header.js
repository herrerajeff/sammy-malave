...
export default function Header( {aboutPhoto, ig, tw, email, openState} ) {
  ...
  return (
    ...
    <button type="button" onClick={toggle} className={`${isOpen ? "bg-black text-white pointer-events-none border border-black py-1 px-3 rounded-full hover:bg-black hover:text-white transition-all duration-200 inline-block" : "pointer-events-auto border border-black py-1 px-3 rounded-full hover:bg-black hover:text-white transition-all duration-200 inline-block bg-white"}`}>About</button>
      ...
      >
        ...
        leaveTo="opacity-0 scale-95">
          <div className="bg-black rounded p-8 relative">
            ...
            <p>
            Curabitur blandit tempus porttitor. Nullam id dolor id nibh ultricies vehicula ut id elit. Duis mollis, est non commodo luctus, nisi erat porttitor ligula, eget lacinia odio sem nec elit. Vivamus sagittis lacus vel augue laoreet rutrum faucibus dolor auctor.</p>
            <p>
            Maecenas sed diam eget risus varius blandit sit amet non magna. Aenean eu leo quam. Pellentesque ornare sem lacinia quam venenatis vestibulum. Praesent commodo cursus magna, vel scelerisque nisl consectetur et. Nulla vitae elit libero, a pharetra augue. Praesent commodo cursus magna, vel scelerisque nisl consectetur et.</p>
            </div>
    ...
    <Menu as="div" className="relative">
      ...
      <Menu.Items className="bg-black rounded-md flex flex-col absolute top-full mt-2 overflow-hidden shadow-xl border border-black">
        <Menu.Item>
        {({ active }) => (
          <a href={`https://www.instagram.com/${ig}`} target="_blank" rel="nofollow noreferrer" className={` ${active ? 'p-4 block bg-white text-black' : 'p-4 block bg-black text-white'} `}>Instagram&nbsp;↗️</a>
          )}
        </Menu.Item>
        <Menu.Item>
        {({ active }) => (
          <a href={`mailto:${email}`} target="_blank" rel="nofollow noreferrer" className={` ${active ? 'p-4 block bg-white text-black' : 'p-4 block bg-black text-white'} `}>Email&nbsp;↗️</a>
          )}
        </Menu.Item>
        <Menu.Item>
        {({ active }) => (
          <a href={`https://www.twitter.com/${tw}`} target="_blank" rel="nofollow noreferrer" className={` ${active ? 'p-4 block bg-white text-black' : 'p-4 block bg-black text-white'} `}>Twitter&nbsp;↗️</a>
          )}
        </Menu.Item>
      </Menu.Items>
      </Transition>
